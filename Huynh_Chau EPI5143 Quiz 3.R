...
# and result from the console window) (hint:use the nrow() and ncol() and/or the dim() R functions)
view(mpg) 
nrow(mpg) 
ncol(mpg) 
dim(mpg)
# There are 234 observations and 11 variables. 
# Nrow=234, Ncol=11, Dim=234 11
...
ggplot (data=mpg) + geom_point(mapping = aes(x=displ, y=hwy, color=class, shape=drv, size=cyl))
ggtitle("Comparing highway fuel efficiency vs. engine size for vehicles")
